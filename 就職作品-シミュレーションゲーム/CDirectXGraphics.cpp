...
bool CDirectXGraphics::Init(HWND hWnd, unsigned int Width, unsigned int Height, bool fullscreen)
{
   ...
   // 機能レベル
	...
	hr = m_lpDevice->CreateTexture2D(&depthBufferDesc, NULL, &m_depthStencilBuffer[0]);
	hr = m_lpDevice->CreateTexture2D(&depthBufferDesc, NULL, &m_depthStencilBuffer[1]);
	if (FAILED(hr)) {
		return false;
	}
	...
	hr = m_lpDevice->CreateDepthStencilView(m_depthStencilBuffer[0].Get(), &depthStencilViewDesc, &m_depthStencilView[0]);
	hr = m_lpDevice->CreateDepthStencilView(m_depthStencilBuffer[1].Get(), &depthStencilViewDesc, &m_depthStencilView[1]);
	...
	m_lpImmediateContext->OMSetRenderTargets(1, m_lpRenderTargetView.GetAddressOf(), m_depthStencilView[RenderTarget].Get());
	...
	vp[0].TopLeftX = 0;
	vp[0].TopLeftY = 0;
	m_lpImmediateContext->RSSetViewports(1, &vp[0]);

	//vp[1].Width = (FLOAT)1000;
	//vp[1].Height = (FLOAT)400;
	//vp[1].MinDepth = 0.0f;
	//vp[1].MaxDepth = 1.0f;
	//vp[1].TopLeftX = 0;
	//vp[1].TopLeftY = 300;
	//m_lpImmediateContext->RSSetViewports(1, &vp[1]);
