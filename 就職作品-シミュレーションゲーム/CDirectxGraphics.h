...
private:
	...
	int                             RenderTarget = 0;                 //レンダリングターゲット

	ComPtr<ID3D11RenderTargetView>	m_lpRenderTargetView;		// レンダーターゲットビュー
	ComPtr<ID3D11Texture2D>			m_depthStencilBuffer[2];		// Ｚバッファ、ステンシルバッファ
	ComPtr<ID3D11DepthStencilState>	m_depthStencilState;		// Ｚバッファ、ステンシルステート
	ComPtr<ID3D11DepthStencilView>	m_depthStencilView[2];			// Ｚバッファ、ステンシルビュー
...
public:
	...
	ID3D11DepthStencilView* GetDepthStencilView() const {
		return m_depthStencilView[RenderTarget].Get();
	}
	...
	void TurnOnZBuffer() {
		...
		m_lpImmediateContext->OMSetRenderTargets(
			1,										// ターゲット
			rtvtable,								// ビューテーブル
			m_depthStencilView[RenderTarget].Get()				// 深度バッファなし
		);
	}
};
